...
@pytest.fixture(scope="module")
def views(guides):
    """Mythicstats aporta una sola vista; las pruebas miran dentro de ella."""
    return [g.views["mythicstats"] for g in guides]


def test_the_data_lands_in_a_source_view(guides):
    assert all(set(g.views) == {"mythicstats"} for g in guides)
...
def test_builds_are_attributed_to_the_dominant_hero_tree(guides):
    ...
    by_hero = {g.hero_name: len(g.views["mythicstats"].talent_builds) for g in guides}
    assert by_hero["Spellslinger"] > by_hero["Frostfire"]


def test_import_strings_look_like_blizzard_loadouts(views):
    for view in views:
...
def test_stat_priority_is_ordered_and_weighted(views):
    stats = views[0].stat_priority
...
def test_gear_and_stats_are_shared_across_hero_trees(views):
    ...
    assert views[0].gear.keys() == views[1].gear.keys()


def test_enchants_carry_their_slot(views):
    slots = {e.slot for e in views[0].enchants}
